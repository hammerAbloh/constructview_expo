...
export default function ObraDetalhe() {
    ...
    if (id) {
      api.getObra(id).then((data) => {
        setObra({
          ...
          previsao: data.previsao || "20/12/2025",
  ...
  if (!obra) return <div className="p-6 text-center">Carregando...</div>;

  let evolucao = [];
  try {
    evolucao = typeof obra.evolucao === 'string' ? JSON.parse(obra.evolucao || '[]') : (obra.evolucao || []);
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        <div>
          <img src={obra.fotos && obra.fotos[0]} alt={obra.nome} className="w-full h-80 object-cover rounded-lg shadow-lg border border-gray-800" />
        ...
        <div className="flex flex-col gap-4">
          <div className="bg-bg-card p-4 rounded-lg border border-gray-800 h-full flex flex-col">
            ...
            <div className="rounded-lg overflow-hidden flex-1 min-h-[300px] border border-gray-800">
              <MapContainer center={[obra.lat, obra.lng]} zoom={15} style={{ height: '100%', width: '100%' }}>
                <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
                <Marker position={[obra.lat, obra.lng]}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        <div className="bg-bg-card p-6 rounded-lg border border-gray-800">
          ...
          <div className="space-y-6">
            {obra.etapas && obra.etapas.map((etapa: any, idx: number) => (
              <div key={idx}>
                ...
                <div className="w-full bg-gray-800 rounded-full h-2">
                  <div className="bg-accent h-2 rounded-full" style={{ width: `${etapa.progresso}%` }}></div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={evolucao} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                />
                <Area type="monotone" dataKey="progresso" stroke="#06b6d4" strokeWidth={3} fillOpacity={1} fill="url(#colorEvolucao)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      {obra.fotos && obra.fotos.length > 1 && (
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {obra.fotos.slice(1).map((foto: string, idx: number) => (
              <img key={idx} src={foto} alt={`Foto ${idx+2}`} className="w-full h-32 object-cover rounded-lg border border-gray-800" />
            ))}
          </div>
        </div>
      )}
